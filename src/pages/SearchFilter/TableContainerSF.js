...
import { Filter, DefaultColumnFilter } from "./FilterSf";
import {
...
const TableContainerSF = ({
  ...
  // const pageSize = 50; // This is your page size
  ...
  const endRange = Math.min(currentPage * pageSize, data?.length);
  ...
  return (
    <Fragment>
      ...
      <Row><Col className="col-7 pt-3"><p> Showing{" "}
      {(pageIndex * pageSize) + 1} to{" "} {Math.min((pageIndex + 1) * pageSize, data.length)} of {data.length} entries</p>
      </Col>
      <Col className="col-5">
        ...
        onClick={() => {
          previousPage();
          onChangeInInput({ target: { value: pageIndex } });
        }}
      ...
      >
        ...
        onClick={() => {
          nextPage();
          onChangeInInput({ target: { value: pageIndex + 2 } });
        }}
...
TableContainerSF.propTypes = {
  preGlobalFilteredRows: PropTypes.any,
};

export default TableContainerSF;