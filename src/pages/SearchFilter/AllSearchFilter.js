...
const AllSearchFilter = ({value,dataToSend,handleLinkClick,resetCounter}) => {
  ...
  const getJobData=()=>{
    ...
    axios
        ...
        const orgDateFormat = org_date?.date_format;

        let apiEndpoint = "/api/list-jobs/data?page=" + pageIndex + "&size=" + pageSize;
        let jobItems =jobStatus.map((item)=>
          item.options
        )
        let ss=jobItems.join(',');
        console.log(ss)

        apiEndpoint += `&job_status=${ss}&created_date_start=${dataToSend.createdDateStart}&created_date_end=${dataToSend.createdDateEnd}&selected_date_format=${orgDateFormat}`;
        axios
            .get(apiEndpoint)
            .then((res) => {
              setJobOrderList(res.data.original.data)
...
const propertiesToSearchJob = filteredJobColumns.map((column) => column.accessor);
...
export default AllSearchFilter