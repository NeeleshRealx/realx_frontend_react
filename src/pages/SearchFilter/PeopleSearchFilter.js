...
const PeopleSearchFilter = ({value,dataToSend,handleLinkClick,resetCounter}) => {
...
const propertiesToSearch = filteredColumns.map((column) => {console.log(column,"ss"); return column.accessor});
console.log(propertiesToSearch)

function filterData(data, searchFilterValue) {
  if (searchFilterValue !== "") {
    return data.filter((item) => {
      // Check if the "job_number" property contains the searchFilter
      const jobNumberValue = item["name"] || ''; // Handle cases where the property is null or undefined
      return jobNumberValue.toString().toLowerCase().includes(searchFilterValue.toLowerCase());
    });
  } else {
    return data;
  }
...
function filterData1(data, searchFilterValue) {
  if (searchFilterValue !== "") {
    return data.filter((item) => {
      // Check if the "job_number" property contains the searchFilter
      const jobNumberValue = item["customer_name"] || ''; // Handle cases where the property is null or undefined
      return jobNumberValue.toString().toLowerCase().includes(searchFilterValue.toLowerCase());
    });
  } else {
    return data;
