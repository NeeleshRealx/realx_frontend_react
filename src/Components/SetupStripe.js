...
const Stripe = ({balance, invoice, onChangeData, button_text, storage_id, email,storage}) => {
  ...
  const handlePayment = async (stripeEmail, amount) =>{
    try {
      ...
      var stripe_one_off_customer_id = (storage.stripe_one_off_customer_id != null) ? stripe_one_off_customer_id : "N";
      ...
      if(result.paymentMethod){
        ...
        axios.post("/api/setupStripe", postData).then((res)=>{
          console.log(res);
          if(res.status == 1){
      ...
      switch (value) { 
        ...
        case "2":
          Swal.fire({
            ...
            <input className="form-control stripe_email" type="text" placeholder="Email" id="stripeEmail" value=${email} disabled style="margin-bottom:10px;width:100%;padding:5px;font-size: 16px;margin-top:30px;"/></div>
...
const StripePayment = ({balance, invoice, onChangeData, button_text, storage_id, email, storage}) =>{
  return (
    <Elements stripe={stripePromise}>
      <Stripe balance={balance} invoice={invoice} onChangeData={onChangeData} storage_id={storage_id} button_text={button_text} email={email} storage={storage}/>
    </Elements>
  );
}
